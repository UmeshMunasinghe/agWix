...
const SOCIAL_PLAYER_YOUTUBE_SDK_TYPE = "$w.YoutubePlayer";
const createSocialPlayerYoutubeSdk = (api) => {
  const { props, setProps } = api;
  return {
    get type() {
      return SOCIAL_PLAYER_YOUTUBE_SDK_TYPE;
    },
    get src() {
      return props.url || "";
    },
    ...
    get showControls() {
      return props.enabledControls ?? false;
    },
    set showControls(value) {
      setProps({ enabledControls: value });
    },
    get showCaptions() {
...
const SocialPlayerYoutubeSdk = composeSDKFactories([
  elementSdkFactory(SOCIAL_PLAYER_YOUTUBE_SDK_TYPE),
  createSocialPlayerYoutubeSdk
]);

export { SocialPlayerYoutubeSdk as default };
